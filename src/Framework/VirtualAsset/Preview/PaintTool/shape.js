...
const mapDispatchToProps = (dispatch, ownProps) => {
    ...
    const rotateMove = (event) => {
        ...
        if (ownProps.paintToolOperation.getIn(['polygon', 'edges']) !== (Math.ceil((angle - 45) / 90) % 4) + 3) {
            dispatch({
                type: 'VIRTUALASSET_PAINTTOOL_SHAPE_POLYGON_ROTATE_CLICK',
    ...
    return {
        selectViewMode: ({ viewMode }) => () => {
            dispatch({
                type: 'VIRTUALASSET_PAINTTOOL_SHAPE_SELECT_VIEWMODE',
                viewMode,
            });
        },
        check: ({ kind, isCheck }) => {
            dispatch({
                type: 'VIRTUALASSET_PAINTTOOL_SHAPE_CHECK',
                kind,
                isCheck,
            });
        },
        colorClick: ({ kind, visibility }) => () => {
            dispatch({
                type: 'VIRTUALASSET_PAINTTOOL_SHAPE_COLORPICKER_VISIBILITY',
                kind,
                visibility,
            });
        },
        colorPicker: ({ color, kind }) => {
            dispatch({
                type: 'VIRTUALASSET_PAINTTOOL_SHAPE_COLORPICKER_SELECT',
                kind,
                color,
            });
        },
        slide: ({ value, slideObject }) => {
            dispatch({
                type: 'VIRTUALASSET_PAINTTOOL_SHAPE_SLIDE',
                value,
                slideObject,
            });
        },
