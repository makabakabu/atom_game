...
const mapStateToProps = (state) => {
    let paintTool = state.getIn(['realAsset', 'paintTool']);
    const focusedStatus = state.getIn(['realAsset', 'content', 'status', 'focusedStatus']);
    if (focusedStatus.get('figureId') !== '') {
        paintTool = state.getIn(['realAsset', 'figuresGroup', focusedStatus.get('figureId'), 'status', focusedStatus.get('statusId'), 'paintTool']);
    }
...
const mapDispatchToProps = dispatch => ({
    ...
    colorClick: ({ viewMode }) => () => {
        ...
        dispatch({
            type: 'PAINTTOOL_PICKER_VISIBILITY',
        });
    },
});
