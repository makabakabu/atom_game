...
const mapDispatchToProps = (dispatch) => {
    const mouseupListener = () => {
        dispatch({
            type: 'PAINTTOOL_CURSOR_ROTATE_ANGLE',
    ...
    return {
        operationMouseDown: ({ viewMode, bracket, direction }) => async () => {
            ...
            if (viewMode === 'location') {
                ...
                dispatch({
                    type: 'PAINTTOOL_MACRO_CURSOR_POSITION_LOCATION_CHNAGE_VALUE',
                    direction,
                });
            } else {
                switch (direction) {
                ...
                dispatch({
                    type: 'PAINTTOOL_CURSOR_POSITION_SIZE_CHNAGE_VALUE',
                    direction,
                    bracket,
                });
            }
        ...
        positionChangeViewMode: ({ viewMode }) => () => {
            dispatch({
                type: 'PAINTTOOL_CURSOR_CHANGE_POSITION_VIEWMODE',
                viewMode,
            });
        },
        bracketSelect: ({ bracket }) => () => {
            dispatch({
                type: 'PAINTTOOL_CURSOR_CHANGE_POSTION_BRACKET',
                bracket,
            });
        },
        selectMode: ({ mode }) => () => {
            dispatch({
                type: 'PAINTTOOL_CURSOR_MODE',
                mode,
            });
        },
        flipSelect: ({ direction }) => () => {
            dispatch({
                type: 'PAINTTOOL_CURSOR_FLIP_SELECT',
                direction,
            });
            dispatch({
                type: 'PAINTTOOL_MACRO_CURSOR_FLIP_SELECT',
                direction,
            });
        },
        onRotate: ({ value }) => {
