...
const cursor = ({
    select: ({ region, mode, row, col, statusId, valueList, value, colLength, rowLength }) => {
        if (!['#ededed', '#cccccc'].includes(value.get('hex')) && !region.getIn([row, col, 'inner'])) {
            region.forEach((rowList, rowIndex) => rowList.forEach((cell, colIndex) => {
                if (cell.get('inner')) {
                    document.getElementById(`${statusId + rowIndex}_${colIndex}`).style.border = 'none';
                }
    ...
    pick: ({ paintToolOperation, row, col, region, valueList, statusId }) => {
        ...
        valueList.forEach((rowList, rowIndex) => {
            rowList.forEach((cell, colIndex) => {
                document.getElementById(`${statusId + rowIndex}_${colIndex}`).style.backgroundColor = `rgba(${parseInt(cell.get('hex').substr(1, 2), 16)},${parseInt(cell.get('hex').substr(3, 2), 16)},${parseInt(cell.get('hex').substr(5, 2), 16)},${cell.get('opacity')})`;
                document.getElementById(`${statusId + rowIndex}_${colIndex}`).style.border = 'none';
            });
        });
