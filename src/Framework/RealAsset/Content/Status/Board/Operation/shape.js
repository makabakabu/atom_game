...
const shape = ({
    select: ({ regionPreCursor, row, col, statusId }) => {
        ...
        document.getElementById(`${statusId + row}_${col}`).style.borderTop = border.includes(1) ? '1px dashed black' : 'none';
        document.getElementById(`${statusId + row}_${col}`).style.borderLeft = border.includes(2) ? '1px dashed black' : 'none';
        document.getElementById(`${statusId + row}_${col}`).style.borderBottom = border.includes(3) ? '1px dashed black' : 'none';
        document.getElementById(`${statusId + row}_${col}`).style.borderRight = border.includes(4) ? '1px dashed black' : 'none';
    },
    pick: ({ startCursor, region, operation, shapeOperation, row, col, statusId }) => {
        // 判断是哪一种模式，
        ...
        region.forEach((rowList, rowIndex) => {
            rowList.forEach((cell, colIndex) => {
                ...
                document.getElementById(`${statusId + rowIndex}_${colIndex}`).style.borderTop = border.includes(1) ? '1px dashed black' : 'none';
                document.getElementById(`${statusId + rowIndex}_${colIndex}`).style.borderLeft = border.includes(2) ? '1px dashed black' : 'none';
                document.getElementById(`${statusId + rowIndex}_${colIndex}`).style.borderBottom = border.includes(3) ? '1px dashed black' : 'none';
                document.getElementById(`${statusId + rowIndex}_${colIndex}`).style.borderRight = border.includes(4) ? '1px dashed black' : 'none';
            });
        });
        return region;
    },
    mouseLeave: ({ region, row, col, statusId }) => {
        if (!region.getIn([row, col, 'inner'])) {
            document.getElementById(`${statusId + row}_${col}`).style.border = 'none';
        } else {
