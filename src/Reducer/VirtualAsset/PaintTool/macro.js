...
const macroReducer = ({ state, action }) => {
    switch (action.type) {
        case 'VIRTUALASSET_PAINTTOOL_MACRO_COLOR':
            return state.set('hex', action.hex);

        case 'VIRTUALASSET_PAINTTOOL_MACRO_CURSOR_SELECT_MOUSEDOWN':
            return state.set('region', action.region);

        case 'VIRTUALASSET_PAINTTOOL_MACRO_CURSOR_FLIP_SELECT': {
        ...
        case 'VIRTUALASSET_PAINTTOOL_MACRO_CURSOR_POSITION_LOCATION_CHNAGE_VALUE':
